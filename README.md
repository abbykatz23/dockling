...
- Poses: idle, bash (construction), edit (coding), search (detective), other-tool, awaiting-input, error, eureka (task completed), thumbs-up (a reply was just sent), committing (bride or groom formalwear, for `git commit` — see [Configuration](#configuration)), pulling (fishing, for `git pull`), butt (farewell — shown for 1.5s right before the duck exits on session end).
